		...
		<form id="pageForm" action="BusiManage/tradingList.action">
			<div class='top-nav-wrap order-top-nav J-state' data-state='0'
				...
				<div class="form-inline">
					<font size="3" face="sans serif">开始时间:</font><input
						style="width: 20%;" type="date" class="form-control" id="name"
						placeholder="请输入开始时间" name="startDate"
						value="${condition.startDate}">
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <font size="3" face="sans serif">结束时间:</font><input
						style="width: 20%;" type="date" class="form-control" id="name"
						placeholder="请输入截止时间" name="endDate" value="${condition.endDate}">
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							...
							<tbody>
								<c:forEach var="i" items="${pageInfo.list}" begin="0"
									...
									<tr>
										...
										<c:choose>
											<c:when test="${i.amountType ==1}">
												<td><c:out value="完成"></c:out></td>
											</c:when>
											<c:when test="${i.amountType ==2}">
												<td><c:out value="违约"></c:out></td>
											</c:when>
										</c:choose>
										<td><c:out value="${i.checkDemands.demandTitle}"></c:out></td>
