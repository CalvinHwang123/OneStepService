	...
	<div class="container">
		<div class="col-md-9 product-w3ls-right">
			...
			<ol class="breadcrumb breadcrumb1" id="breadcrumb1">
				...
				<li class="active"><a
					href=<%=path + "/Portal/workInfoList.action"%>>作品</a></li>

			</ol>
			<!-- 搜索框 -->
			&nbsp;&nbsp;&nbsp;
			<div class="header-search">
				<form action=<%=path + "/Portal/workInfoList.action"%>
					id="workslook" method="post">
					<input type="hidden" id="beWorksPrice" name="beWorksPrice"
						value=171120 title="${beWorksPrice}" /> <input type="hidden"
						id="endWorksPrice" name="endWorksPrice" value=171120
						title="${endWorksPrice}" /> <input type="hidden" id="classPid"
						name="classPid" value=171120 title="${classPid}" /> <input
						type="hidden" id="startDate" name="startDate" value=""
						title="${checkbox}" />
					...
					<input type="hidden" id="currentPageInput" name="pageNum"
						value="${pageInfo.getPageNum()}" /> <input id="classname"
						name="title" placeholder="按作品名搜索" required="" type="search"
						title="${looktitle}">
					<button id="classnamelook" type="submit" class="btn btn-default"
		...
		<!-- 右部div -->
		<!-- 左部div -->
		<form action=<%=path + "/Portal/classpricevaluelook.action"%>
			method="post">
					...
					<div class="slider-left">
						...
						<div class="row row1 scroll-pane jspScrollable"
							...
							<div class="jspContainer" style="width: 223px; height: 160px;">
								<div id="jspPane" class="jspPane"
									style="padding: 0px; width: 246px; top: 0px;"
									title="${worksPrice}">
									<label class="checkbox"><input class="racheckbox"
										name="checkbox" type="radio" value="1-10000"> <i></i>￥1
										- ￥10000 </label> <label class="checkbox"><input
										class="racheckbox" name="checkbox" type="radio"
										value="10000-30000"> <i></i>￥10000 - ￥30000 </label> <label
										class="checkbox"><input class="racheckbox"
										name="checkbox" type="radio" value="30000-80000"> <i></i>￥30000
										- ￥80000 </label> <label class="checkbox"><input
										class="racheckbox" name="checkbox" type="radio"
										value="80000-1000000"> <i></i>More</label>
								</div>
							</div>
						</div>
					</div>
					...
					<div class="sidebar-row">
						...
						<ul class="faq">
							<c:forEach var="i" items="${oneclassmenulist}">
								<li class="item1"><a><c:out
											value="${i.classificationName}"></c:out><span
										class="glyphicon glyphicon-menu-down"></span></a>
									<ul style="display: none;">
										<c:forEach var="j" items="${twoclassmenulist}">
											<c:if test="${j.classificationPid==i.classificationId}">
												<li><a class="twoclassname"
													value="${j.classificationId}"
													title="${j.classificationName}"><c:out
															value="${j.classificationName}"></c:out></a></li>
											</c:if>
										</c:forEach>
									</ul></li>
							</c:forEach>
