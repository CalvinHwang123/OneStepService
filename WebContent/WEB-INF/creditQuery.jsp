...
<body>
	...
	<div class="header-search">
		<form id="pageForm" action="Portal/creditQuery.action" method="post">
			...
			placeholder="请输入雇主名或服务商名" required="">
		...
		$(function() {
			$("#input_search").on('input propertychange', function() {
				...
				if (searchKey.length > 0) {
					$.ajax({
			  			url: $("base").attr("href") + "Portal/creditQuerySuggest.action",
			  			type:"post",
			  			dataType:"text",
			  			contentType : "application/json;charset=utf-8", //如果采用requestbody那么一定要加
			  			data:JSON.stringify({"userName": searchKey}),
			  			async:true,
			  			success:function(msg){
			  				var userNameListArr = JSON.parse(msg);
			  				$("#search_suggest").html("<ul>");
			  				for (var i = 0; i < userNameListArr.length; ++i) {
				  				$("#search_suggest").append("<li><a href='javascript:void(0)' onclick='submitSearch(this)'>" + userNameListArr[i] +  "</a></li>");
			  				}
			  				$("#search_suggest").append("</ul>");
			  			}
			  		});
				} else {
					$("#search_suggest").html("");
