						...
						<c:forEach var="i" items="${pageInfo.list}" begin="0"
							...
							<tr>
								...
								<td><c:out value="${status.index+1}"></c:out></td>
											<td>
											<c:forEach var="j" items="${listUser}">
											<c:if test="${j.userID==i.userId }">
											<c:out value="${j.userName}"></c:out>
											</c:if>
											</c:forEach>
											</td>
											...
											<c:out value="定金"></c:out>
											</c:if>
											<c:if test="${i.amountType==2}">
											<c:out value="尾款"></c:out>
											</c:if>
											<c:if test="${i.amountType==3}">
											<c:out value="退款"></c:out>
											</c:if>
											</td>
